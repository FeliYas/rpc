...
@section('content')
                    ...
                    <tbody class="bg-gray-100" id="facturaTableBody">
                        @forelse($facturas as $factura)
                            <tr class="hover:bg-gray-200 transition-colors duration-200 factura-row"
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-700">
                                    ${{ number_format($factura->tipo_cambio, 2) }}</td>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div class="mb-4">
                            <label for="edit_iva_monto" class="block text-sm font-medium text-gray-700 mb-1">Monto IVA
                                (USD)</label>
                            <input type="number" id="edit_iva_monto" name="iva_monto" step="0.01"
                                class="w-full border border-gray-300 px-4 py-2 rounded-md bg-gray-100" readonly>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="mb-4">
                            <label for="edit_subtotal" class="block text-sm font-medium text-gray-700 mb-1">Subtotal
                                (USD)</label>
                            <input type="number" id="edit_subtotal" name="subtotal" step="0.01"
                                class="w-full border border-gray-300 px-4 py-2 rounded-md bg-gray-100" readonly>
                        </div>
                        ...
                        <div class="mb-4">
                            <label for="edit_importe_total" class="block text-sm font-medium text-gray-700 mb-1">Importe
                                Total (ARS)</label>
                            <input type="number" id="edit_importe_total" name="importe_total" step="0.01"
                                class="w-full border border-gray-300 px-4 py-2 rounded-md bg-gray-100 font-bold text-lg"
                                readonly>
                        </div>
                    </div>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-900">$<span
                                            id="totalGravadoUSD">0.00</span></td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-900">$<span
                                ...
                                <tr class="bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-right font-bold text-gray-900">
                                        $<span id="totalGeneralUSD">0.00</span></td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-right font-bold text-gray-900">
    ...
    <script>
        ...
        function calcularTotalEdit() {
            ...
            const tipoCambio = parseFloat(document.getElementById('edit_tipo_cambio').value) || 0;
        ...
        function mostrarResumenFacturas() {
            ...
            checkedBoxes.forEach(checkbox => {
                ...
                // Obtener datos de la factura desde el JSON embebido
                ...
                if (facturaData && facturaData.detalles && facturaData.detalles.length > 0) {
                    ...
                    const tipoCambio = parseFloat(facturaData.tipo_cambio);

                    // Sumar montos en USD
                    totalGravadoUSD += parseFloat(detalle.gravado);
                    totalIvaUSD += parseFloat(detalle.iva_monto);

                    // Calcular y sumar montos en ARS usando el tipo de cambio individual
                    totalGravadoARS += parseFloat(detalle.gravado) * tipoCambio;
                    totalIvaARS += parseFloat(detalle.iva_monto) * tipoCambio;
            ...
            const totalGeneralUSD = totalGravadoUSD + totalIvaUSD;

            // Llenar la modal con los datos
            document.getElementById('totalGravadoUSD').textContent = totalGravadoUSD.toFixed(2);
            document.getElementById('totalGravadoARS').textContent = totalGravadoARS.toFixed(2);
            document.getElementById('totalIvaUSD').textContent = totalIvaUSD.toFixed(2);
            document.getElementById('totalIvaARS').textContent = totalIvaARS.toFixed(2);
            document.getElementById('totalGeneralUSD').textContent = totalGeneralUSD.toFixed(2);
            ...
            listaFacturas.innerHTML = facturasInfo.map(factura =>
                `<div class="flex justify-between items-center p-2 bg-white rounded border">
                    ...
                    <div class="text-right">
                        <div class="text-sm text-gray-500">${new Date(factura.fecha).toLocaleDateString('es-AR')} - TC: $${factura.tipoCambio.toFixed(4)}</div>
                        <div class="font-medium">$${factura.totalARS.toFixed(2)}</div>
                    </div>
                </div>`
            ).join('');
